...
| `src/styles/leaderboard.css` | Players leaderboard (`/player`, `.community`) in the Practice look: a card per leaderboard with its icon white on a tile in its own color, medals for the top three, and the online players in a sticky side card with rating chips. Tournament winners (`/tournament/leaderboard`, `.tournament-leaderboards`) reuse the same cards, with a gold / silver / bronze cup per yearly / monthly / weekly winner, the tournament as a pill, and the side menu and title pinned while the cards scroll. |
