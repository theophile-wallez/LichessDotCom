...
| `src/styles/tv.css` | Lichess TV (`main.round.tv-single`): the game page's layout with the channels back in a scrolling column left of the board, each a row with its mode's icon white on a gradient tile in the leaderboards' colors, the champion and a rating chip, the channel on air with a live dot (`content.js` scrolls it into view). Below 1260px only the tiles, their names as tooltips. |
